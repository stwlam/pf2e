...
import { darknessLevelAtTime, DarknessTransition, intervalToTransition } from "./animate-darkness.ts";
...
export class WorldClock extends fa.api.HandlebarsApplicationMixin(fa.api.ApplicationV2) {
    ...
    async syncDarkness(scene = canvas.scene, { animate = scene?.isView ?? false, timeDiff = 1 } = {}): Promise<void> {
        ...
        type DateTimeTuple = [DateTime, DateTime];
        const timePairs = transitionTimes.reduce((pairs: DateTimeTuple[], dateTime) => {
    ...
    get darknessSynced(): boolean {
        return canvas.scene?.darknessSyncedToTime ?? false;
    }
    ...
    #initialize() {
        ...
        const { dawnTime, duskTime } = setting;
        TimeOfDay.DAWN = new TimeOfDay(dawnTime.hour, dawnTime.minute);
        TimeOfDay.DUSK = new TimeOfDay(duskTime.hour, duskTime.minute);
